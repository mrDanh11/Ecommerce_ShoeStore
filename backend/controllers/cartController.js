...
const deleteCartItem = async (req, res) => {
    ...
    try {
        // giấy giỏ hàng hiện tại
        const { cart, error: errorCart } = await Cart.getCartByCustomer(customerId);
        ...
        const cartId = cart.magiohang
        // lấy sp có itemId trong Chitietgiohang -> xoá
        const { cartItem, error: errorCartItem } = await Cart.getCartItemById(itemId)
        console.log('check cartItem: ', cartItem)
        if (errorCartItem || !cartItem) {
            return res.status(404).json({
                success: false,
                error: "Không tìm thấy sản phẩm trong giỏ hàng"
            })
        }
        if (cartItem[0].magiohang !== cartId) {
            return res.status(404).json({ success: false, error: 'Sản phẩm không thuộc giỏ hàng này' });
        }
        const { error: errorDeleteCartItem } = await Cart.deleteCartItem(itemId)
        if (errorDeleteCartItem)
            throw errorDeleteCartItem;
        // tính lại tổng tiền & tổng số lượng
        const { items: allItems, error: errorAllItems } = await Cart.getAllCartItems(cartId);
        if (errorAllItems) throw errorAllItems;
        ...
        const { error: errorUpdateCart } = await Cart.updateCartTotals(cartId, tongsoluong, totalamount)
        if (errorUpdateCart)
