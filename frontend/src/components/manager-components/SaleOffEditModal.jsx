...
const SaleOffEditModal = ({ isOpen, onClose, saleOff, onSave }) => {
  ...
  // Initialize form data when modal opens or saleOff changes
  useEffect(() => {
    if (saleOff) {
      setFormData({
        value: saleOff.giatri || "",
        status: saleOff.trangthai ?? true,
        startDate: saleOff.ngaybatdau ? dayjs(saleOff.ngaybatdau).format("YYYY-MM-DD") : "",
        endDate: saleOff.ngayketthuc ? dayjs(saleOff.ngayketthuc).format("YYYY-MM-DD") : "",
      })
      setErrors({})
    }
  }, [saleOff])
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await onSave(saleOff.masaleoff, {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Chỉnh sửa mã giảm giá</h2>
          <button
            onClick={handleClose}
            disabled={loading}
...
export default SaleOffEditModal
